...
def _process_patient(patient_idx: int) -> dict[str, int]:
    patient_id = f"{patient_idx:02d}"
    ...
    with load_patient_summary(patient_id, DataConfig.dataset_path) as patient_summary:
        (
            preictal_intervals,
    ...
    if is_precomputed_data_exists(data_path=patient_stfts_dir):
        ...
        logger.info(f"Memory size: {loaded_raw.get_data().nbytes / (1024**2):.2f} MB")
...
def main():
    ...
    with logging_redirect_tqdm(loggers=active_loggers):
        if DataConfig.patient_to_process >= 1:
            precomputed_stfts_summary.append(
                _process_patient(DataConfig.patient_to_process)
            )
        else:
            for idx in tqdm(
                range(1, DataConfig.number_of_patients + 1), desc="Patients"
            ):
                precomputed_stfts_summary.append(_process_patient(idx))
