...
from src.preprocessing.check_image import (
    load_npz_data,
    plot_bispectrum,
    plot_spectrogram,
)
...
def predict_from_uploaded(
    ...
    try:
        tf_t = preprocess_for_model(
            tf_tensor_np, expected_size=expected_size, device=device
        )
        bis_t = preprocess_for_model(
            bis_tensor_np, expected_size=expected_size, device=device
        )
    except Exception as e:
    ...
    with torch.no_grad():
        try:
            ...
            try:
                ...
                return (
                    f"Model forward failed (dual-input and concat tried): {e}",
                    "0.00%",
                )
        except Exception as e:
...
def main():
    with gr.Blocks(
        ...
        with gr.Row(elem_classes="title-center"):
            gr.Markdown(
                "# Seizure Prediction Using EfficientNetB0+CBAM With Gated Fusion"
            )
        ...
        with gr.Row(elem_classes="model-note-center"):
            gr.Markdown(
                "#### *Note: The model expects that the uploaded test cases are of the same patient used during training.*"
            )
        ...
        with gr.Row():
            ...
            with gr.Column(scale=2):
                model_upload = gr.UploadButton(
                    label="Upload model (.pt)", file_types=[".pt"]
                )
        ...
        with gr.Row(elem_classes="input-note-center"):
            gr.Markdown(
                "#### *Note: Uploaded time-frequency and bispectrum files must be of the same patient and same epoch.*"
            )
        ...
        with gr.Group():
            with gr.Row(elem_classes="inputs-row"):
                with gr.Column(elem_classes="input-col"):
                    ...
                    tf_input = gr.UploadButton(
                        label="Upload Time-Frequency File", file_types=[".npz"]
                    )
                    tf_filename = gr.Textbox(
                        label="Selected Time-Frequency File", interactive=False
                    )
                    tf_plot = gr.Plot(label="Spectrogram Preview")
                    tf_input.upload(
                        on_tf_upload, inputs=tf_input, outputs=[tf_filename, tf_plot]
                    )
                with gr.Column(elem_classes="input-col"):
                    gr.Markdown("### Input Bispectrum File (.npz)")
                    bis_input = gr.UploadButton(
                        label="Upload Bispectrum File", file_types=[".npz"]
                    )
                    bis_filename = gr.Textbox(
                        label="Selected Bispectrum File", interactive=False
                    )
                    bis_plot = gr.Plot(label="Bispectrum Preview")
                    bis_input.upload(
                        on_bis_upload,
                        inputs=bis_input,
                        outputs=[bis_filename, bis_plot],
                    )
